...
- Baseline kiểm thử hiện tại: `47 tests OK`.
...
- Nhánh thử nghiệm `experiment/html-ebook-reader` đã merge vào `main`: reader có thể dùng nội dung HTML đã lọc và hiển thị ảnh minh họa an toàn.
- Đã polish dashboard cho demo: sửa text không dấu, đồng bộ nút/bảng/form/badge và empty state ở các trang quản trị chính.
...
- Reader hỗ trợ nội dung HTML đã sanitize và ảnh minh họa từ nguồn Gutenberg khi có dữ liệu `content_html`.
...
- Dashboard đã được polish tiếng Việt, nút thao tác, bảng dữ liệu và trạng thái rỗng để demo gọn hơn.
...
- Reader ebook render HTML đã lọc, giữ ảnh hợp lệ và loại script nguy hiểm.
...
- Dashboard quản trị đã được làm sạch text/nút/bảng/form, phù hợp demo cuối kỳ hơn.
- Chatbot đã giảm hallucination bằng DB-first search.
- Test tăng lên 47 case.
...
1. Smoke test thủ công toàn bộ luồng demo trên trình duyệt thật trước khi thuyết trình.
2. Thêm e2e/browser test cho cart, checkout, wishlist, dashboard role nếu còn thời gian.
3. Tách `views.py` theo domain sau demo.
